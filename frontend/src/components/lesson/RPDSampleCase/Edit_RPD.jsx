...
if (filePattern && filePattern.file) { // ใช้ `filePattern.file`
  await deleteOldFile(existingModel.patternUrl); // ลบไฟล์ pattern เก่า
  patternUrl = await uploadFile(filePattern.file, 'patterns'); // อัปโหลดไฟล์ pattern ใหม่
}

if (fileImage && fileImage.file) { // ใช้ `fileImage.file`
  await deleteOldFile(existingModel.imageUrl); // ลบไฟล์รูปภาพเก่า
  imageUrl = await uploadFile(fileImage.file, 'images'); // อัปโหลดไฟล์ภาพใหม่
}
